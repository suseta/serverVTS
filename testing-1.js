...
async function enqueueData(decodedData) {
...
async function processQueue() {
  const decodedData = await dequeueData();
  if (decodedData) {
    storeDataInDb(decodedData);
    console.log('Processed:', decodedData);
  }
}
setInterval(processQueue, 1000); // Process queue every 1 seconds
